package com.mohamed.NinjaGold.controllers;
...
public class MainController {
	...
	public String index(HttpSession session, @RequestParam(value = "farm", required = false) String farm,
		...
		if (farm != null) {
			...
			gold += amount;
			activities.add(0,
					"You entered a farm and earned " + amount + "gold . (" + simpleDateFormat.format(new Date()) + ")");
			session.setAttribute("activities", activities);
			session.setAttribute("gold", gold);
			return "redirect:/";
		}
		if (cave != null) {
			...
			gold += amount;
			activities.add(0,
					"You entered a cave and earned " + amount + "gold . (" + simpleDateFormat.format(new Date()) + ")");
			session.setAttribute("activities", activities);
			session.setAttribute("gold", gold);
			return "redirect:/";
		}
		if (house != null) {
			...
			gold += amount;
			activities.add(0,
					"You entered a house and earned " + amount + "gold . (" + simpleDateFormat.format(new Date()) + ")");
			session.setAttribute("activities", activities);
			session.setAttribute("gold", gold);
			return "redirect:/";
		}
